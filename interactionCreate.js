    ...
    async execute(interaction, client) {
        ...
        if (interaction.isChatInputCommand()) {
            ...
            try {
                ...
                const errorMessage = {
                    content: '**حدث خطأ أثناء تنفيذ الملف!**',
        ...
        if (interaction.isModalSubmit()) {
            ...
            if (customId === 'apply_modal_full') {
                const dept = fields.getTextInputValue('dept') || (fields.getStringSelectValues ? fields.getStringSelectValues('dept')[0] : null);
            ...
            if (customId.startsWith('apply_reject_modal_')) {
                ...
                const embed = EmbedBuilder.from(interaction.message.embeds[0])
                    .setColor('#f04747')
                    .setTitle('❌ [تم الرفض]')
                    .addFields({ name: 'سبب الرفض', value: reason });
